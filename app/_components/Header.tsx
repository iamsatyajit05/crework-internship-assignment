...
export default function Header() {
    ...
    return (
        <nav className="w-full sticky sm:static top-0 z-[100] bg-background">
            <div className="max-w-screen-xl w-full m-auto p-5 flex justify-between items-center flex-wrap md:flex-nowrap gap-5">
                ...
                <div>
                    <button className="space-y-1.5 outline-none block md:hidden" onClick={() => setMenuOpen(!menuOpen)}>
                        {menuOpen
                ...
                <div className={`${menuOpen ? 'block' : 'hidden'} grow md:grow-0 md:flex gap-10`}>
                    <ul className="flex flex-col items-center md:flex-row flex-1 gap-5">
                        {links.map(({ href, label }, index) => (
                            <li key={index}>
                                <Link href={href} className="text-[#BDBCBC] hover:text-white transition-all text-xl">
                                    {label}
                                </Link>
                            </li>
                        ))}
